...
**必须使用 Python 3.13**（`FlagEmbedding` / `bge-m3` 强依赖）。
...
> ⚠️ 启动前务必设置 `export OMP_NUM_THREADS=1`（Windows：`set OMP_NUM_THREADS=1`），
> 否则 `bge-m3` / `FlagEmbedding` 在多线程 OpenMP 下偶发 torch 段错误。
...
```bash
# 启动后看日志，应出现「Numpy 模式」字样
export OMP_NUM_THREADS=1
uvicorn backend.main:app --host 0.0.0.0 --port 8000

# 调用知识库健康检查接口
curl http://localhost:8000/api/kb/health
# 返回 mode 应为 "numpy"，chunk_count 应为 30532
#（34154 条原始 chunk 过滤非中英文后剩余 30532 条可用）
```
...
> 读一下 `README.md` 和 `docs/metrics_validation_report.md`

我会读到完整上下文——项目结构、已经做了什么、还剩什么、代码改了哪。
