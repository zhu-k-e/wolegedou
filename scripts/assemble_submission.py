...
def main():
    ...
    with open(quick, "w", encoding="utf-8") as f:
        f.write(
            ...
            "================================================\n\n"
            "本包为【自包含】提交包，解压后模型与知识库已内置（无需外网下载）。\n\n"
            "★ 最简部署（评委只需这一条命令，脚本会自动建虚拟环境、装依赖、生成 .env）★\n"
            "   1. 安装 Python 3.13（https://www.python.org/）与 Node.js 18+（https://nodejs.org/）\n"
            "   2. 在解压出的 wolegedou 目录里执行：\n"
            "        python scripts/start_all.py\n"
            "   3. 浏览器打开 http://localhost:5176 即可看到完整系统：\n"
            "       创建学习任务 → 多智能体调度可视化 → 资源生成。\n"
            "   （首次运行会自动 pip install 依赖，需联网约 1-3 分钟；模型/知识库已离线内置。）\n\n"
            "────────────────────────────────────────────────────────\n"
            "如需手动分步（等价，一般不必要）：\n"
            "一、环境准备\n"
            "      cd wolegedou\n"
            ...
            "      cp .env.example .env\n"
            "      编辑 .env，填入 DEEPSEEK_API_KEY 与 OPENAI_API_KEY（或 DASHSCOPE_API_KEY）\n\n"
            "三、启动\n"
            "   同时拉起前端+后端： python scripts/start_all.py\n"
            "   仅后端（无界面）：   python scripts/start_server.py\n\n"
            "四、验证\n"
