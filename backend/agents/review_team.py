...
class Verifier(BaseAgent):
    ...
    def system_prompt(self) -> str:
        return (
            ...
            "对每条知识点，结合系统检索好的知识库Top-3片段与你自身的专业知识进行判断。\n"
            "\n"
            "【fact_accuracy 评分标准（必须严格遵守）】\n"
            "fact_accuracy = 事实正确的陈述数 ÷ 全部陈述数，是一个客观比例，不是主观印象分。\n"
            "  - 全部陈述均正确 → 1.0\n"
            "  - 10 条中 1 条错误 → 0.9\n"
            "  - 半数错误 → 0.5\n"
            "判定单条陈述时遵循：\n"
            "  1. 陈述与知识库片段一致 → 正确。\n"
            "  2. 知识库片段未覆盖该陈述，但依据你的专业知识判断其正确 → **仍记为正确**。\n"
            "     知识库覆盖范围有限，「检索不到」不等于「错误」，不得因此扣分。\n"
            "  3. 仅当陈述与知识库片段矛盾，或依据专业知识确认其错误时 → 记为错误。\n"
            "禁止在无法判断时一律返回 0.5 —— 若确实无从判断，按上述第 2 条记为正确。\n"
            "\n"
