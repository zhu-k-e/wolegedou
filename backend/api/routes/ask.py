...
def _save_resource_stats(session_id: str, result: dict) -> None:
    ...
    只要 quiz 或 lecture 任一存在即写入（即使 quiz 无 questions 或 lecture.difficulty_note 为空），
    保证 /api/report 的 difficulty_match.points 不会因空数据而无法构建曲线。
    """
    import json
    ...
    if lecture and isinstance(lecture, dict):
        lecture_note = lecture.get("difficulty_note", "") or "基础"

    # 只有 quiz 和 lecture 都不存在时才跳过（兜底保证曲线有点）
    if not quiz and not lecture:
        return
    ...
    # 兜底：dispatch_info 缺失/为空时，退化用画像 domain_hint 首项，避免写入 domain=NULL
    if not domain:
        hint = profile.get("domain_hint") or []
        domain = hint[0] if hint else None
