...
async def get_report(session_id: str) -> LearningReport:
    ...
    # 兜底（bug #1 修复 2026-08-27）：session 尚无实测资源（新任务落库前/时序竞态）时，
    # 基于画像 domain_confidence 领域生成基线点，保证 difficulty_match.points 不为空、
    # 前端曲线可绘制。student_level 取真实置信度分值；resource_difficulty 无实测，
    # 退化为学生水平基线（曲线退化为水平线），不冒充实测数据。
    if not rows:
        fallback_note = "暂无资源难度实测数据，曲线为学情画像基线"
        baseline_domains = list(domain_confidence.keys()) or (
            profile.get("domain_hint") or []
        )
        rows = [
            {"domain": d, "quiz_difficulties": None} for d in baseline_domains
        ]
    ...
    match_curve = DifficultyMatchCurve(
        points=points, overall_match_rate=overall_rate, note=fallback_note or None
    )
